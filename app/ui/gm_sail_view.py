...
# from app.db.models import User, Fief
...
#         self.setup_data = setup_data  # Contains dest, gold, ships, waypoints
...
#                 units_input=self.cargo.value,  # Loading new troops
#                 commander=self.commander.value or None,
#                 gold_to_carry=self.setup_data["gold"],
#                 waypoints=self.setup_data["waypoints"],
#                 is_gm_override=True,  # <--- GM FLAG
#                 acting_house_id=self.target_house_id,  # <--- NPC HOUSE
#             )
...
#         # Check if fleet already has troops loaded
...
#         # PATH B: Fleet is empty -> Check location
#         async with get_session() as session:
...
#                 )
#                 await interaction.response.send_message(
#                     f"⚓ **GM Logistics:** Fleet is at Friendly Port. Provision House {self.target_house_id} troops?",
#                     view=view,
#                     ephemeral=True,
#                 )
#             else:
#                 await interaction.response.send_message(
#                     f"⛔ **GM Check:** Fleet is empty and not at a friendly port.\nCannot provision new troops here.",
#                     ephemeral=True,
#                 )
...
#             # FIX: Use the specific coordinate columns
...
#     """Shared function to send GM feedback + Fog of War"""
#     if not success:
#         return await interaction.followup.send(
#             f"❌ GM Command Failed: {result}", ephemeral=True
#         )

#     # 1. Private GM Feedback
#     embed = discord.Embed(
...
class GMSailSetupModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # --- FIX 2: Check the state flag before processing any new gold input ---
        gold_amount = 0
