...
class SailSetupModal(discord.ui.Modal, title="Issue Sailing Orders"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.has_cargo:
            ...
            async with get_session() as session:
                ...
                success, result, fog_msg = await service.sail_fleet(
                    ...
                    ships_input=ships_input,
                    dest_name=dest,
                    units_input=None,  # No new units requested, fleet already has cargo
                    commander=None,
        ...
        # 2. IF FLEET IS EMPTY -> Always proceed to cargo selection
        #    The previous check for 'is_at_friendly_fief' is removed.
        modal2_data = {
            "ships": ships_input,
            "gold": gold_amount,
            "destination": dest,
        ...
        await interaction.response.send_message(
            "⚓ **Logistics:** Click below to provision crew/cargo for your empty fleet.",
            view=view,
            ephemeral=True,
        )
