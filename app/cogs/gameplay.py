...
class GameplayCog(commands.Cog):
    ...
    @commands.command(name="reset_quarters")
    @commands.has_permissions(administrator=True)
    async def reset_quarters(self, ctx, target: discord.Member):
        """
        GM Tool: Clears a player's locked private channel ID.
        Use this if a player deleted their channel and can't use !me.
        """
        async with get_session() as session:
            game = await GameRepo.get_active_game(session, ctx.guild.id)
            stmt = (
            ...
            if player:
                player.private_channel_id = None  # Wipe the ID
                await session.commit()
                await ctx.send(
                    f"✅ cleared private channel lock for **{target.display_name}**. They can now use `!me` in their quarters to re-lock it."
                )
            else:
