import random
import datetime
import asyncio  # Keep asyncio imported for clarity, though explicit calls might not be in BattleService directly now
...
class BattleService:
    ...
    # --- HELPER: STOP MOVEMENT (PREVENTS RACE CONDITIONS) ---
    ...
    async def _get_home_fief(self, house_id: int) -> Fief | None:
        """Async version to get home fief."""
    ...
    async def resolve_manual_battle_aftermath(self, battle_id: int) -> tuple[str, int]:
        ...
        battle = (await self.session.execute(stmt)).scalars().first()
        if not battle:
            # Consistent return for error case, includes guild_id as None if battle not found
            return "Error: Battle not found.", None

        attacker, defender = battle.attacker, battle.defender
        ship_capacity = battle.game.ship_capacity  # guild_id is accessed here
        guild_id = battle.game.guild_id  # Define guild_id explicitly here
        ...
        if loser_id_to_delete:
            try:
                ...
                stmt_del_army = delete(Army).where(Army.army_id == loser_id_to_delete)
                await self.session.execute(stmt_del_army)
                await self.session.commit()
            except Exception:
    ...
    # ======================================================================
    # ===== AUTO-BATTLE METHODS (For Celery Tasks, now ASYNC) =============
    # ======================================================================

    # Renamed from _get_character_martial_sync and made async
    # The async _get_character_martial already exists above, so we'll ensure all calls use that one.
    # We remove the redundant _sync version.

    # Renamed from _get_home_fief_sync and made async
    # The async _get_home_fief helper is now available above.

    async def start_auto_battle(
        self, game_id, attacker_id, defender_id, ambush, defense
    ):
        """
        Starts an auto-resolved battle round (e.g., from Celery).
        """
        attacker = await self.session.get(Army, attacker_id)
        defender = await self.session.get(Army, defender_id)
        if not attacker or not defender:
            return None, "Armies not found.", None

        battle_type = (
            "LAND_BATTLE"  # Assuming land for auto-battle start unless explicit.
        )
        att_martial = await self._get_character_martial(
            attacker.house_id, attacker.commander_name
        )
        ...
        await self.session.commit()
        return new_battle, f"Attacker Odds: 1 - {odds}", "Auto-battle started."

    async def process_auto_battle_round(self, battle_id: int):
        """
        Processes a round of an auto-resolved battle.
        """
    ...
    async def resolve_auto_battle_aftermath(
        ...
        """
        Handles casualties, cargo loss, cleanup, and RE-EMBARKING for auto-resolved battles.
        """
        # 1. Fetch Battle
        battle = (
            (
        ...
        if attacker:
            await self.session.refresh(attacker)
        if defender:
            await self.session.refresh(defender)
        ...
        if winner and winner.house:
            winner_house = await self.session.get(
                House, winner.house_id
            )  # Using await session.get
            if winner_house:
                winner_house.treasury += loot
        if loser:
            loser.treasury = 0
        ...
        if winner.army_type == "SEA":
            stmt_ghost = select(Army).where(  # Using select() for async session
                Army.house_id == winner.house_id,
                Army.army_type == "LAND",
                Army.commander_name == winner.commander_name,
                Army.status.in_(["MARCHING", "IDLE"]),
            ...
            ghosts = (
                (await self.session.execute(stmt_ghost)).scalars().all()
            )  # Await execute
            for g in ghosts:
                print(
                ...
                kill_task(g)
                await self.session.delete(g)  # Await delete
    ...
    async def start_siege(
        ...
        2. Eagerly loads 'fief' to prevent UI crashes.
    ...
    async def occupy_fief(
        ...
        GM_OVERRIDE: If True, bypasses direct player ownership check and uses acting_house_id.
        """
        ...
        # Determine the effective house that is performing the action
        effective_commanding_house_id: int | None = None
        if is_gm_override:
            if acting_house_id is not None:
                effective_commanding_house_id = acting_house_id
            else:
                # If GM is overriding but didn't specify acting_house_id,
                # assume they mean the army's current house.
                effective_commanding_house_id = army.house_id
        else:
            # Regular player logic: Find the player and their claimed house
            ...
            if not player or not player.claimed_house_id:
                return (
                    False,
                    "❌ You do not command a house or not registered for this game.",
                )
        ...
        # Authority check: Ensure the army belongs to the effective commanding house
        if army.house_id != effective_commanding_house_id:
            # For regular players, this is a direct denial.
            # For GMs, it means the specified 'acting_house_id' (or inferred house)
            # does not match the army's actual owner.
            return (
                False,
                f"❌ This army (ID: {army_id}, House ID: {army.house_id}) does not belong to the commanding House ID {effective_commanding_house_id}.",
            )
        ...
        # If the effective commanding house already owns the fief, just garrison it
    ...
    # ======================================================================
    # ===== SYNCHRONOUS METHODS (Used by Celery Tasks) =====================
    # ======================================================================

    def _get_character_martial_sync(self, house_id: int, commander_name: str) -> int:
        """Synchronous version of martial check."""
        if not commander_name:
            return 0
        stmt = select(Character).where(
            Character.house_id == house_id, Character.name.ilike(commander_name)
        )
        # Using self.session directly (sync)
        char = self.session.execute(stmt).scalars().first()
        return char.skills.get("martial", 0) if char and char.skills else 0

    def start_battle_sync(self, game_id, attacker_id, defender_id, ambush, defense):
        """
        Synchronous version of start_auto_battle.
        Returns: (Battle object, odds_msg, calculation_log)
        """
        attacker = self.session.get(Army, attacker_id)
        defender = self.session.get(Army, defender_id)
        if not attacker or not defender:
            return None, "Armies not found.", None
    ...
    def process_battle_round_sync(self, battle_id: int):
        """
        Synchronous version of process_auto_battle_round.
        Returns: (battle, message, winner, phase_transition_boolean)
        """
    ...
    def resolve_aftermath_sync(self, battle_id: int):
        """
        Synchronous version of resolve_auto_battle_aftermath.
        Returns: (final_report_string, guild_id) or (None, None)
        """
        # 1. Fetch Battle
        stmt = (
            select(Battle)
        ...
        battle = self.session.execute(stmt).scalars().first()
        ...
        loot = loser.treasury if loser else 0
        if winner and winner.house:
            winner_house = self.session.get(House, winner.house_id)
            if winner_house:
                winner_house.treasury += loot
        if loser:
            loser.treasury = 0
        ...
        if winner.army_type == "SEA":
            ...
            ghosts = self.session.execute(stmt_ghost).scalars().all()
            for g in ghosts:
                print(
                    f"[BATTLE] Found Winner's Ghost Army {g.army_id}. Re-embarking survivors."
                )
        ...
        # For normal auto-battles (no Siege support yet in auto-mode), delete battle
        ...
        if loser_id_to_delete:
            try:
                ...
                stmt_del_army = delete(Army).where(Army.army_id == loser_id_to_delete)
                self.session.execute(stmt_del_army)
                self.session.commit()
            except Exception:
