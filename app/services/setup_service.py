...
class SetupService:
    ...
    async def init_world(
        ...
        house_name_to_id = {}
        heir_house_name = None  # Track this dynamically for Pass 2
        fiefs_created = 0
        ...
        for entry in world_data:
            ...
            # Resolve Dynamic Naming
            if raw_house_name == "[CROWN]":
                house_name = f"{ruling_house_name}"
            elif raw_house_name == "[CROWN_HEIR]":
                house_name = f"Prince of {fief_name}"
                heir_house_name = house_name  # Save for Pass 2 routing
            elif raw_house_name == ruling_house_name and fief_name in [
                "Storm's End",
                "Summerhall",
            ]:
                house_name = f"{ruling_house_name} of {fief_name}"
            else:
            ...
            fief = Fief(
                ...
                integration=(
                    0.10 if is_ruined else 1.0
                ),  # Set initial integration for EconomyService
            )
            self.session.add(fief)
            fiefs_created += 1

            # Army Creation (Garrisons/Fleets)
            all_stats = entry.get("army_stats", {})
            if all_stats and not is_ruined:  # Ruins shouldn't start with armies
                land_comp = {
        ...
        for entry in world_data:
            ...
            # Resolve Liege Name (FIXED: No longer hardcoded to Dragonstone)
            if liege_name == "[CROWN]":
