...
class WarfareService:
    ...
    async def march_army(
        ...
        gold_to_carry: int = 0,
        waypoints: str | None = None,
        travel_mode_req: str = "land_only",  # <-- SOLUTION for Banner Call flexibility
    ):
        """
        ...
        if not await self._check_command_authority(player, source_army):
            return (
                False,
        ...
        player_house = await self.session.get(House, player.claimed_house_id)
        if gold_to_carry > player_house.treasury:
            return (
                False,
                f"❌ Not enough gold! You have {player_house.treasury}, tried to take {gold_to_carry}.",
                None,
            )

        player_house.treasury -= gold_to_carry
        ...
        gate_info, gate_fief, cut_index = await self._check_gate_interception(
            game_id, player.claimed_house_id, path_points
        )
        if gate_info:
    ...
    async def stop_march(
        self, game_id: int, user_id: int, army_id: int, is_admin: bool = False
    ):
        army = await ArmyRepo.get_army_by_id(self.session, army_id)
        if not army:
    ...
    async def split_army(
        self, game_id: int, user_id: int, army_id: int, split_amount: int, new_name: str
    ):
        """
        ...
        if not await self._check_command_authority(player, army):
            return (
                False,
    ...
    async def form_coalition(
        ...
        leader_player = (await self.session.execute(stmt)).scalars().first()
        ...
        for army in armies_to_merge:
            if not bypass_auth:
                if not await self._check_command_authority(leader_player, army):
                    return (
                        False,
                        f"❌ You do not have command authority over **{army.commander_name}**.",
                    )
        ...
        coalition = Army(
            game_id=game_id,
            house_id=leader_player.claimed_house_id,
    ...
    async def disband_coalition(self, game_id: int, user_id: int, army_id: int):
        """
        Disbands a coalition, correctly restoring the original contingents,
        including their treasuries, to their owners.
        """
        # 1. --- VALIDATION (This part is already correct) ---
        stmt_p = select(GamePlayer).where(
            GamePlayer.user_id == user_id, GamePlayer.game_id == game_id
        )
        player = (await self.session.execute(stmt_p)).scalars().first()
        ...
        if not await self._check_command_authority(player, coalition_army):
            return (
                False,
                f"❌ You do not have command authority over the **{coalition_army.commander_name}** coalition.",
            )

        # 2. --- FETCH CONTINGENT DATA (This part is already correct) ---
        stmt_contingents = (
        ...
        for contingent in contingents:
            ...
            restored_army = Army(
                ...
                treasury=contingent.treasury or 0,  # <-- CORE FIX: RESTORE THE GOLD
    ...
    async def merge_armies(self, game_id: int, user_id: int, id_1: int, id_2: int):
        """
    ...
    async def embark_army(
        self, game_id: int, user_id: int, land_army_id: int, fleet_id: int
    ):
        ...
        if (
            not player
            or land_army.house_id != player.claimed_house_id
            or fleet.house_id != player.claimed_house_id
        ):
    ...
    async def disembark_army(self, game_id: int, user_id: int, army_id: int):
        """
        ...
        if not player or fleet.house_id != player.claimed_house_id:
        ...
        land_status = (
            "GARRISONED" if fief and fief.owner_id == fleet.house_id else "IDLE"
        )
    ...
    async def recruit_troops(
        self, game_id: int, user_id: int, fief_name: str, amount: int
    ) -> tuple[bool, str]:  # <--- Added return type hint for clarity
        """
        Turns Manpower into Troops at a specific Fief, respecting the game's
        manpower rules.
        """
        # <--- CHANGE #1: Fetch the Game object to check the ruleset.
        game = await self.session.get(Game, game_id)
        if not game:
            # This is an internal check, should not happen in normal play
            return False, "❌ Internal error: Game not found."

        # 1. Validation (This part remains the same)
        stmt_p = (
            select(GamePlayer)
            .where(GamePlayer.user_id == user_id, GamePlayer.game_id == game_id)
            .options(selectinload(GamePlayer.house))
        ...
        # <--- CHANGE #2: Wrap manpower logic in a conditional block.
        manpower_message = "Manpower was not a factor."
        if game.manpower_enabled:
            # --- This is the ORIGINAL logic, which now only runs if the rule is ON ---
        ...
        # Find Fief (This part remains the same)
        ...
        # 3. Create Composition based on Region (This part remains the same)
        comp_ratios = {
            ...
            # Regions NOT present in the screenshot → set to None
        ...
        # 4. Add to Garrison (This part remains the same)
        ...
        # <--- CHANGE #3: Use the dynamic message in the final output.
        return (
            True,
    ...
    def _find_closest_coastal_landing(
        ...
        """
        Scans outward from an inland target to find the nearest coastal water tile
        ...
        """
        # Ensure we have access to the cost map
        ...
        # Directions: Up, Down, Left, Right (and diagonals if you want, but 4 is safer for "adjacent")
        directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]

        # Define what counts as water/port
        # Assuming you have these constants available. Adjust if they are named differently.
        ...
        while queue:
            ...
            for dx, dy in directions:
                ...
                if 0 <= ny < rows and 0 <= nx < cols:
                    ...
                    # FOUND WATER?
                    if terrain_cost in WATER_TILES:
                        # We found a water tile (nx, ny).
                        # The tile we came from (cx, cy) MUST be land (or we wouldn't be walking on it).
                        # So: Fleet goes to (nx, ny), Army lands at (cx, cy).
                        return (nx, ny), (cx, cy)

                    # If it's valid land, add to queue to keep searching
                    # Check if it's passable land (not obstacle/impassable)
                    if terrain_cost < 255:  # Assuming 255 is impassable
                        visited.add((nx, ny))
                        queue.append((nx, ny))
    ...
    async def sail_fleet(
        ...
        - Auto-detects coastal landing spots for inland destinations (The Arbor fix).
        - Auto-picks up standing armies (Ghost Ship fix).
        - Ensures troops spawn on land, not water.
        """
        # =================================================================
        ...
        if not await self._check_command_authority(player, source_fleet):
            return (
                False,
        ...
        # =================================================================
        # 2. PATHFINDING & AUTO-COAST DETECTION
        # =================================================================
        origin_name = await self.get_location_name_from_coords(
        ...
        dest_is_water_or_port = self._is_coord_water_or_port(
            int(dest_coords["x"]), int(dest_coords["y"])
        )

        path_data = None

        # --- BRANCH A: INLAND DESTINATION (e.g., The Arbor) ---
        if not dest_is_water_or_port:
            # If user didn't provide waypoints, we AUTO-SEARCH for the coast
            if not parsed_waypoints:
                print(
                    f"[DEBUG SAIL] Destination {dest_name} is inland. Searching for nearest coast..."
                )
                ...
                if not best_water_loc:
                    return (
                        False,
                        f"❌ Could not find a suitable landing spot near **{dest_name}**. The island may be too far inland or mapped incorrectly. Please try specifying a coastal waypoint.",
                        None,
                    )

                # A. Sea Path: Start -> Best Water Loc
                sea_path = await PF_ENGINE.find_journey_async(
                    start_loc=start_coords,
                ...
                if not sea_path:
                    return (
                        False,
                        f"❌ Cannot sail to the coast near {dest_name}.",
                        None,
                    )

                # B. Land Path: Best Land Loc -> Final Destination
                land_path = await PF_ENGINE.find_journey_async(
                ...
                if not land_path:
                    return (
                        False,
                        f"❌ Cannot march from the landing spot to {dest_name}.",
                        None,
                    )

                # C. Combine Results
                path_data = sea_path
                path_data["path_points"].extend(land_path["path_points"][1:])
                path_data["total_distance"] += land_path["total_distance"]
                for k, v in land_path["terrain_breakdown"].items():
                    path_data["terrain_breakdown"][k] = (
                        path_data["terrain_breakdown"].get(k, 0.0) + v
                    )

                # SAVE COORDINATES FOR LATER
                path_data["auto_landing_water"] = best_water_loc
                path_data["auto_landing_land"] = best_land_loc

                # Image generation
                all_stops = [
                    start_coords,
                    best_water_loc,
                    (dest_coords["x"], dest_coords["y"]),
                ]
                ...
                # User provided waypoints for inland destination (Manual Mode)
                # ... (Insert your existing manual hybrid logic here if desired, or allow it to fall through) ...
                return (
                    False,
                    "❌ To use auto-landing, please remove manual waypoints for inland destinations.",
                    None,
                )

        # --- BRANCH B: WATER/PORT DESTINATION ---
        else:
            path_data = await PF_ENGINE.find_journey_async(
                start_loc=start_coords,
                end_loc=(dest_coords["x"], dest_coords["y"]),
        ...
        if not path_data:
            return (False, "❌ No viable path found.", None)

        # =================================================================
        # 3. DETERMINE CARGO & TROOPS (WITH PICKUP FIX)
        # =================================================================
        try:
            ship_amount = (
                int(ships_input)
            ...
            if ship_amount <= 0 or ship_amount > source_fleet.troop_count:
                raise ValueError(
                    f"Invalid ship count. You have {source_fleet.troop_count} ships."
                )

            total_men, cargo_payload = 0, {}

            # --- CASE A: RECRUIT NEW (Requires Fief) ---
            if units_input is not None and units_input.lower() != "load":
                stmt_loc = select(Fief).where(
                    ...
                    Fief.owner_id == player.claimed_house_id,
                )
                if not (await self.session.execute(stmt_loc)).scalars().first():
                    return (
                        False,
                        "❌ **Logistics Error:** You can only draft NEW troops at your own Fiefs. To pick up an existing army, just type `!sail [ships] [dest]`.",
                        None,
                    )
                ...
                capacity = ship_amount * ship_capacity
                if parsed_men > capacity:
                    raise ValueError(
                        f"Over Capacity: {ship_amount} ships can only hold {capacity} men."
                    )

                total_men, cargo_payload = parsed_men, {
                    "commander": commander or f"Captain of {player.house.name}",
                    "troop_count": parsed_men,
                    "composition": specific_comp,
                }

            # --- CASE B: EXISTING CARGO ---
            elif source_fleet.cargo and source_fleet.cargo.get("troop_count", 0) > 0:
                total_men = source_fleet.cargo.get("troop_count", 0)
                cargo_payload = copy.deepcopy(source_fleet.cargo)

            # --- CASE C: PICK UP ARMY (The Fix) ---
            else:
                ...
                if ground_army:
                    capacity = ship_amount * ship_capacity
                    if ground_army.troop_count > capacity:
                        return (
                            False,
                            f"❌ Not enough ships! Army has {ground_army.troop_count} troops, need more capacity.",
                            None,
                        )

                    total_men = ground_army.troop_count
                    cargo_payload = {
                        "commander": ground_army.commander_name,
                        "troop_count": ground_army.troop_count,
                        "composition": ground_army.composition,
                    }
                    await self.session.delete(ground_army)  # Remove from ground
                else:
                    return (
                        False,
                        "❌ **Ghost Ship Error:** No cargo on board, and no army found at this location to pick up.",
                        None,
                    )
        ...
        if ship_amount < source_fleet.troop_count:
            fleet_to_sail = await ArmyRepo.split_army_logic(
                ...
                source_fleet,
                ship_amount,
                commander or f"Detachment of {source_fleet.commander_name}",
            )
        ...
        # 5. HANDLE GOLD
        # =================================================================
        if gold_to_carry > player.house.treasury:
            return (
                False,
                f"❌ Not enough gold! You have {player.house.treasury}.",
                None,
            )
        player.house.treasury -= gold_to_carry
        fleet_to_sail.treasury = (fleet_to_sail.treasury or 0) + gold_to_carry

        # =================================================================
        # 6. JOURNEY CALCULATION
        # =================================================================
        now = datetime.datetime.now(datetime.timezone.utc)
        path_land_dist = path_data["terrain_breakdown"].get("land", 0) + path_data[
            "terrain_breakdown"
        ].get("road", 0)
        ...
        if path_land_dist > 5 or "auto_landing_water" in path_data:

            # Use Auto-Found Coords if available
            if "auto_landing_water" in path_data:
                disembark_water = path_data["auto_landing_water"]
                disembark_land = path_data["auto_landing_land"]
                disembark_name = "Coastal Landing"
            else:
                # Fallback for manual hybrid paths
                disembark_water, _ = self._find_disembarkation_point(
                    path_data["path_points"]
                )
                disembark_land = disembark_water  # Fallback (risky for water error, but needed for legacy)
                disembark_name = "The Coast"

            # 1. Fleet sails to WATER tile
            fleet_to_sail.destination_x = disembark_water[0]
            fleet_to_sail.destination_y = disembark_water[1]

            sea_dist = path_data["terrain_breakdown"].get("sea", 0)
            fleet_dur = calculate_travel_duration({"sea": sea_dist}, ship_amount)
            fleet_arr = now + datetime.timedelta(seconds=fleet_dur)

            fleet_to_sail.departure_time = now
            fleet_to_sail.arrival_time = fleet_arr
            fleet_to_sail.status = "SAILING"

            journey_summary = f"Sailing to **{disembark_name}**, then marching overland to **{dest_name}**."

            # 2. Ghost Army created at LAND tile (Crucial Fix)
            land_army = await ArmyRepo.create_embarked_army(
                self.session,
                fleet_to_sail,
                dest_coords["x"],
                dest_coords["y"],
            ...
            # FORCE LOCATION TO VALID LAND TILE
            land_army.location_x = disembark_land[0]
            land_army.location_y = disembark_land[1]

            # Clear cargo to avoid duplication
            cargo_payload = None
            fleet_to_sail.cargo = None

            land_dur = calculate_travel_duration({"land": path_land_dist}, total_men)
            land_army.arrival_time = fleet_arr + datetime.timedelta(seconds=land_dur)
            final_duration = (land_army.arrival_time - now).total_seconds()

        # --- PURE SEA EXECUTION ---
        else:
            journey_summary = f"Sailing directly to **{dest_name}**."
            final_duration = calculate_travel_duration(
                path_data["terrain_breakdown"], ship_amount
            )
            fleet_to_sail.destination_x, fleet_to_sail.destination_y = (
                dest_coords["x"],
                dest_coords["y"],
            )
            fleet_to_sail.departure_time, fleet_to_sail.arrival_time = (
                now,
                now + datetime.timedelta(seconds=final_duration),
            )
            fleet_to_sail.status = "SAILING"
        ...
        await self.session.flush()
        if land_army:
            land_army_id = land_army.army_id

        stmt = (
            update(Army)
            .where(Army.army_id == fleet_to_sail.army_id)
            .values(cargo=cargo_payload)
        )
        await self.session.execute(stmt)
        await self.session.commit()
        ...
        if land_army_id:
            land_army.task_id = resolve_army_arrival.apply_async(
                args=[land_army_id], eta=land_army.arrival_time
            ).id
        # --- B. PLAYER INTERACTION / INTERCEPTION SYSTEM (NEW) ---
        # This calculates if the fleet will collide with anyone while sailing.
        # --- B. PLAYER INTERACTION (FIRST COLLISION ONLY) ---
        from app.tasks.light_tasks import initiate_player_interaction

        # Determine check duration
        if land_army_id:
            check_duration = (fleet_to_sail.arrival_time - now).total_seconds()
        else:
            check_duration = final_duration
        ...
        # 1. Filter for valid SEA enemies only
        unique_enemy_ids = {c["enemy_id"] for c in collisions}
        valid_sea_ids = set()
        ...
        # 3. Pick the FIRST valid sea contact
        first_contact = None
        for col in collisions:
            if col["enemy_id"] in valid_sea_ids:
                first_contact = col
                break  # Stop searching after finding the first valid enemy

        # 4. Schedule ONLY that one
        if first_contact:
            prompt_time = first_contact["time"] - datetime.timedelta(hours=1)
            if prompt_time < now:
                prompt_time = now + datetime.timedelta(seconds=15)
            ...
            print(
                f"[DEBUG SAIL] Scheduled FIRST Naval Interception with Fleet {first_contact['enemy_id']} at {first_contact['time']}"
            )
        ...
        return (
            True,
            {
                ...
                "time": format_duration(final_duration),
    ...
    def _find_disembarkation_point(self, path_points):
        ...
        """
        # In WarfareService.__init__, you'd have: self.cost_map = PF_ENGINE.cost_map
    ...
    async def redirect_army(
        ...
        if not player or not await self._check_command_authority(
            player, army_to_redirect
        ):
            return False, "❌ You do not have authority over this unit.", None

        # 2. Stop the current movement
        stop_success, stop_msg = await self.stop_march(game_id, user_id, army_id)
        ...
        if army_to_redirect.army_type == "LAND":
            # Note: Removed 'travel_mode_req' as discussed previously
        ...
        # --- Fleet Redirect ---
            ...
            success, result, fog_msg = await self.sail_fleet(
                ...
                ships_input="all",
                dest_name=new_dest_name,
                units_input=None,  # This will now use the RESTORED cargo
                commander=None,
                waypoints=new_waypoints,
            )
            if success:
    ...
    async def occupy_fief(self, game_id: int, user_id: int, army_id: int):
        ...
        # --- FIX: Join User table to match Discord ID ---
        ...
        if not player:
            return False, "❌ You do not have a registered player account in this game."
